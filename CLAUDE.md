...
- All purchases go through a **Benosaurus iframe** pointing at `https://benosaurus.cz/objednavka/plk1-8d005281?embed=1&teamy=1&studenti=30`. The iframe renders the full order form directly inline (no modal): konfigurátor (slider týmů, počet studentů, ceny, **teploměr zábavnosti**), fakturační údaje, a podle počtu týmů buď „Zaplatit kartou" (Stripe via aharta wrapper) + sekundární „Chci fakturu místo toho" pro 1 tým, nebo „Odeslat objednávku" (faktura, ARES autofill IČO) pro 2+ týmů. Benosaurus is built by **Roman Věžník**. There is **no longer a separate `.plk-cta` Stripe button on the page** — the iframe is the only purchase entry point. Stripe URL je v Benosaurus adminu, ne hardcoded zde.
