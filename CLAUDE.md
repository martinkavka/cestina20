...
- **Benosaurus order widget** — JS from `benosaurus.pages.dev/embed/kviz.js` (async). Renders into the host DOM (same-origin), so host CSS can reach inside if needed. Supports `data-font="inherit"`, `data-bare="1"`, `data-button-uppercase="1"`, `data-button-shadow="..."`, and `data-accent` (flat hex color applied to the CTA background and slider thumb). The Stripe card-payment URL is set in the Benosaurus admin (not hardcoded here). Currently `data-accent="#319f39"` is set because the admin's green gradient doesn't fully propagate into the inline widget — the flat green is a closer match to the theme `.button` than the widget's default blue. Some `data-*` attributes may not behave as documented (e.g. `data-bare` has been observed not to fully strip the card chrome); the widget is a black box on the Benosaurus side, so visible behavior can shift without changes in this repo — if something looks regressed, ping Roman before touching HTML.
- **Benosaurus infrastructure note** — Roman migrated Benosaurus from Cloudflare Pages to Workers in April 2026. The **older iframe embed at `naveromag.cz/benosaurus/...` is permanently dead**; if ever reverting to the iframe variant, use `https://benosaurus.veznik.workers.dev/benosaurus/objednavka/plk1-8d005281?embed=1&compact=1`. Around 22–23 April 2026 Benosaurus gets its own domain `benosaurus.cz` (then `https://benosaurus.cz/objednavka/plk1-8d005281?...`). **Do not switch back to the iframe variant though** — it lacks the unified Stripe-for-1-team / invoice-for-2+-teams flow that the JS widget provides.
